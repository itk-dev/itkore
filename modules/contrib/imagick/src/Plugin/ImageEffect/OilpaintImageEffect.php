...
/**
 * @file
 * Contains \Drupal\imagick\Plugin\ImageEffect\OilpaintImageEffect.
 */
...
class OilpaintImageEffect extends ConfigurableImageEffectBase {
  ...
  public function applyEffect(ImageInterface $image) {
    if (!$image->apply('oilpaint', $this->configuration)) {
      $this->logger->error('Image oilpaint failed using the %toolkit toolkit on %path (%mimetype)', array('%toolkit' => $image->getToolkitId(), '%path' => $image->getSource(), '%mimetype' => $image->getMimeType()));
      return FALSE;
    }
    return TRUE;
  }
